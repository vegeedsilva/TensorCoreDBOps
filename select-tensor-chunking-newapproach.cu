...
int main(int argc, char* argv[]) {
   int condition = 1; //selection condition default to 1 
   long long size;
   printf("Enter matrix size in powers of two ");
   scanf("%lld", &size);
   cublasHandle_t cublasHandle;
   long long MATRIX_M, MATRIX_N, MATRIX_K;
   long long CHUNK_MATRIX_SIZE = 1024; // chunk matrix is of dimension 2^20 i.e. 2^10 * 2^10 matrix
   long long CHUNK_MATRIX_M = 1048576; // chunk size is taken as 2^20
   MATRIX_M = size;
   MATRIX_N = size;
   MATRIX_K = size;
   ...
   for(int i=0; i<MATRIX_M; i++){
    ...
    if(MATRIX_M > CHUNK_MATRIX_M){
          ...
          float *h_c = new float[CHUNK_MATRIX_M];
          for(long long i=0; i<MATRIX_M;i++){
            h_a[i] = 0;
            h_b[i] = 0;
          }

          for(long long i=0; i<CHUNK_MATRIX_M;i++){
            h_c[i] = 0;
          }
          ...
          for(int i =0; i < noOfLoops; i++){
                ...
                convertFp32ToFp16 <<< (MATRIX_M + 255) / 256, 256 >>> (d_a_half, d_a, MATRIX_M , start, end);
                convertFp32ToFp16 <<< (MATRIX_M + 255) / 256, 256  >>> (d_b_half, d_b, MATRIX_M, start, end);
                ...
                cublasErrCheck(cublasGemmEx(cublasHandle, CUBLAS_OP_N, CUBLAS_OP_N, 
                              ...
                              &alpha,
                              d_a_half, CUDA_R_16F, CHUNK_MATRIX_SIZE,
                              d_b_half, CUDA_R_16F, CHUNK_MATRIX_SIZE,
                              &beta, 
                 ...
                 if(condition == 1)
                   condition_equal<<< (CHUNK_MATRIX_SIZE * CHUNK_MATRIX_SIZE + 255) / 256, 256 >>>(d_c,d_g, start);
                 else if(condition == 2)
					          condition_lessthan<<<  (CHUNK_MATRIX_SIZE * CHUNK_MATRIX_SIZE + 255) / 256, 256 >>>(d_c,d_g, start);
                 else
					          condition_greaterthan<<<  (CHUNK_MATRIX_SIZE * CHUNK_MATRIX_SIZE + 255) / 256, 256 >>>(d_c,d_g, start);
          ...
          printf("Total execution time %fms", totaltime);
          ...
          convertFp32ToFp16 <<< (matrix_size*matrix_size + 255) / 256, 256 >>> (d_a_half, d_a, matrix_size* matrix_size, start, end);
          convertFp32ToFp16 <<< (matrix_size*matrix_size + 255) / 256, 256 >>> (d_b_half, d_b, matrix_size* matrix_size, start, end);
          ...
          cublasErrCheck(cublasGemmEx(cublasHandle, CUBLAS_OP_N, CUBLAS_OP_N, 
                        ...
                        &alpha,
                        d_a_half, CUDA_R_16F, matrix_size,
                        d_b_half, CUDA_R_16F, matrix_size,
                        &beta, 
            ...
            /* insert the launch parameters to launch the kernel properly using blocks and threads */ 
           if(condition == 1)
              condition_equal<<< (matrix_size*matrix_size + 255) / 256, 256 >>>(d_c,d_g, start);
           else if(condition == 2)
			        condition_lessthan<<< (matrix_size*matrix_size + 255) / 256, 256 >>>(d_c,d_g, start);
           else
              condition_greaterthan<<< (matrix_size*matrix_size + 255) / 256, 256 >>>(d_c,d_g, start);
